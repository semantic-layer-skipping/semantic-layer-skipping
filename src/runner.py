...
class SemanticSkipRunner:
    ...
    def __init__(self, model_name: str = "Qwen/Qwen2.5-1.5B-Instruct", *,
        ...
        Args:
            checkpoints (list[int]): Points (layer indices) where skip decisions are evaluated.
        ...
        logging.info(f"Initialised with {len(self.checkpoints)} checkpoints: {self.checkpoints}")
    ...
    def infer_and_populate(self,
                           prompt: str,
                           vector_db: SkippingVectorDB, *,
                           early_exit_strategy: Optional[EarlyExitStrategy] = None,
                           skip_strategy_mode: Optional[SkipStrategyMode] = SkipStrategyMode.COSINE,
                           similarity_threshold: float = 0.95
                           ) -> str:
        """
        Runs full inference to find ground truth, then checks every checkpoint to see if we could have Exited or Skipped.
        Populates the DB with positive examples.

        DB should have number of layers equal to the number of decision points.
        Uses the early_exit_strategy when deciding whether we should populate the vector db with an early exit decision.
        Uses the skip_strategy_mode to determine whether to use cosine similarity or strict match for skip decisions, which this method implements.
        ...
        for checkpoint_idx, layer_idx in enumerate(self.checkpoints):
            ...
            if early_exit_strategy:
                ...
                if should_exit:
                    ...
                    vector_db.add_vector(checkpoint_idx, current_state.detach().cpu().numpy().reshape(1, -1), decision)
            ...
            for future_checkpoint_idx in range(len(self.checkpoints) - 1, checkpoint_idx, -1):
                ...
                if skip_strategy_mode == SkipStrategyMode.COSINE:
                    # compare Input(Current) vs Input(Target) - which means we can skip to Input(Target)
                    target_state = full_cache[f"blocks.{target_layer_idx}.hook_resid_pre"][0, -1, :]
                    sim = F.cosine_similarity(current_state, target_state, dim=0).item()
                    ...
                    with torch.no_grad():
                        sim_logits = self.model.run_with_hooks(
                            tokens,
                            fwd_hooks=[(f"blocks.{target_layer_idx}.hook_resid_pre", injection_hook)]
                        )
                ...
                if is_valid_skip:
                    ...
                    decision = SkipDecision(action=Action.SKIP, skip_count=skip_n_checkpoints)

                    vector_db.add_vector(checkpoint_idx, current_state.detach().cpu().numpy().reshape(1, -1), decision)
                    logging.info(
                        f"  [L{layer_idx}] Added SKIP to L{target_layer_idx}")
                    break  # found the biggest skip, stop searching for this start of checkpoint

        return target_token_str

    def infer_with_skipping(self,
                            prompt: str,
                            vector_db: SkippingVectorDB,
                            threshold: float = 0.7
                            ) -> str:
        ...
        Threshold is the cosine similarity threshold for deciding whether to skip: if the nearest neighbor's similarity is above this threshold, we take its decision.
        Returns the final prediction.
        """
        ...
        def checkpoint_hook(resid_pre, hook):
            ...
            if result:
                logging.info(f"  [L{layer_idx}] Retrieved from DB {result}, Threshold: {threshold:.2f}]")
                ...
                if similarity < threshold:
                    ...
                    if target_ckpt_idx < len(self.checkpoints):
                        ...
                        logging.info(f"  [L{layer_idx}]  SKIPPING to L{target_layer} (Checkpoint {target_ckpt_idx}).")
        ...
        logging.info(f"Final Prediction: '{pred_str}' | Skipped Layers: {ctx.skipped_layers_count}")
        return pred_str
