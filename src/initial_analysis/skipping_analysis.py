...
class SkipAnalyser:
    ...
    def analyse_skips(
        ...
        logging.info(
            f"Baseline Target Token: '{target_token_str}' (ID: {target_token_id})"
        )
        ...
        for start_layer in range(n_layers - 1):
            ...
            start_state = full_cache[f"blocks.{start_layer}.hook_resid_post"][
                0, -1, :
            ]  # shape: (hidden_size,)
            ...
            for n in range(1, max_skip_size + 1):
                ...
                cos_similarity = functional.cosine_similarity(
                    start_state, end_state, dim=0
                ).item()
                passes_similarity = cos_similarity >= similarity_threshold

                # --- Strategy B: Strict Match Simulation ---
                # we inject start_state directly into 'dest_layer',
                # bypassing the middle.
                def injection_hook(resid_pre, hook, state=start_state):
                ...
                with torch.no_grad():
                    ...
                    # TODO: slice model and
                    #  only run from dest_layer onwards for efficiency
                    sim_logits = self.model.run_with_hooks(
                        tokens, fwd_hooks=[(hook_point, injection_hook)]
                    )
                ...
                if passes_strict:
                    logging.info(
                        f"✅ FOUND: L{start_layer}->Skip{n} (Sim: {cos_similarity:.4f})"
                    )
                elif passes_similarity:
                    logging.info(
                        f"⚠️ High Sim ({cos_similarity:.4f}) but "
                        f"Strict Failed L{start_layer}->Skip{n}"
                    )
                else:
                    # did not pass either check
                    logging.info(
                        f"❌ No Skip Valid at L{start_layer}->Skip {n}"
                        f" (Sim: {cos_similarity:.4f})"
                    )
...
def plot_skip_heatmap(
    results: list[dict[str, Any]], prompt: str, max_skip_size: int, n_layers: int
):
    """
...
if __name__ == "__main__":
    ...
    for prompt in prompts:
        ...
        plot_skip_heatmap(
            results,
            prompt,
            max_skip_size=max_skips,
            n_layers=analyser.model.cfg.n_layers,
        )
        ...
        if successful_skips:
            ...
            for res in successful_skips[:10]:
                print(
                    f"Start L{res['start_layer']} "
                    + f"-> Skip {res['skip_count']}"
                    + f" -> Land L{res['land_layer']} "
                    + f"(Sim: {res['similarity']:.4f})"
                )
        else:
